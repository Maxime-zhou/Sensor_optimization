...
%!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
% Section 3.7.9
% Project: capacitance exctraction 
%!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
%!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
exercise_sec030709_readgmsh_capacitance  % input file is read   
...
%-----------------------------------------------------------------------------------------
 ...
 for i1=1:length(Ie)
  ...
  F(I)=F(I)-Ke(p,LeD)*Ue(LeD);             % assemblage of rhs
 end 
...
cap=0.d0;
for e=1:analysis.NE,                     % capacitance computation
 type=elements(e).type;
 ne=Le(type).ne;                         % number of nodes in element
 Etag=Le(type).tag;
 Dne=ndof*ne;                            % number of nodal values in one surface el.
 ...
 for n=1:ne
  ...
  if nodes(node).dof(1)<0 & nodes(node).U(1)==0
   We(pos:pos+ndof-1)=1;                 % sets We=1 on electrode with phi=0
  end 
  pos=pos+ndof;
 end
 mat=elements(e).mat;
