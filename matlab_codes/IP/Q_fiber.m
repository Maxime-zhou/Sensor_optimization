...
theta = [0, a, pi/4, pi/2, pi-a, pi, pi+a, 3*pi/2, 2*pi-a, 2*pi];
...
if(~all(P_ind))  % if any element of P_ind equal to zero, Q is zero
    ...
    dDdp(i,:) = ( (dUdp(2*P_ind(i)-1,:) - dUdp(2*P_ind(i)-1,:)) * cos(theta(L(i))) + ...
                      (dUdp(2*P_ind(i),:) - dUdp(2*P_ind(i),:)) * sin(theta(L(i))) ) / dL;
    end
    Q = det(dDdp'*dDdp);
end
