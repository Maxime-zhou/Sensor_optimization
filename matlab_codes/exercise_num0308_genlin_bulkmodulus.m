...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% EXERCISE: numerical estimate of bulk modulus
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

press=1;                                 % imposed press
work=0.d0;
for e=1:analysis.NE,                     
 type=elements(e).type;
 ne=Le(type).ne;                         
 ...
 for n=1:ne
  node=elements(e).nodes(n);   
  Xe(n,:)=nodes(node).coor;              % creates element
  Ue(pos:pos+ndof-1)=nodes(node).U;      % list of nodal value
  pos=pos+ndof;
 end
 mat=elements(e).mat;
 Ke=eval([Etag Atag 'Ke(Xe,material(mat,:))']);       
...
H=10;
V=10;
vol=pi*H^2*V;                            % volume of half cube
R=5;
vol1=2/3*pi*R^3;                         % volume of half sphere
f=vol1/vol;                               % fraction

DV=2*pi*work/press;                      % volume variation
Ka_num=press*vol/DV                          % numerical estimate
...
nu2=material(2,2);
K2=E2/(3*(1-2*nu2));
Ka_1=(1-f)*K2+f*K1                           % first estimate

Ka_2=K2*(1-pi/6+(pi/6-f)/(1+3*(1+nu2)/ ...   % refined estimate
       (pi*(1-2*nu2))*f))  
